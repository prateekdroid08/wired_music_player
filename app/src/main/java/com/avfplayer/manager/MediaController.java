...
import com.avfplayer.dbhandler.MostAndRecentPlayTableHelper;
...
public class MediaController implements NotificationManager.NotificationCenterDelegate, SensorEventListener {
    ...
    public void playPreviousSong() {
        ...
        currentPlaylistNum--;
        info.setIndex(currentPlaylistNum);
    ...
    public boolean playAudio(SongDetail mSongDetail) {
        ...
        if (MusicPreferance.playingSongDetail != null) {
            Intent intent = new Intent(ApplicationAVFPlayer.applicationContext, MusicPlayerService.class);
            ApplicationAVFPlayer.applicationContext.startService(intent);
        } else {
            Intent intent = new Intent(ApplicationAVFPlayer.applicationContext, MusicPlayerService.class);
            ApplicationAVFPlayer.applicationContext.stopService(intent);
        }

        storeResendPlay(ApplicationAVFPlayer.applicationContext, mSongDetail);
        NotificationManager.getInstance().notifyNewSongLoaded(NotificationManager.newaudioloaded, mSongDetail);

        return true;
    }

    public void playNextSong(boolean byStop) {
        ArrayList<SongDetail> currentPlayList = shuffleMusic ? MusicPreferance.shuffledPlaylist : MusicPreferance.playlist;
    ...
    public boolean pauseAudio(SongDetail messageObject) {
        ...
        if (messageObject != null)
            Log.d("Stop Song Now:", messageObject.toString());
        else
            Log.d("Stop Song Now:", "Message object is null now");
    ...
    public int getSeekTO() {
        ...
        if (audioPlayer != null) {
            seekTime = audioPlayer.getCurrentPosition();
        }
        return seekTime;
    }
    ...
    public void cleanupPlayer(Context context, boolean notify, boolean stopService) {
        ...
        SongDetail songDetail = getPlayingSongDetail();

        AllAudioVideoInfo info = new AllAudioVideoInfo();


        if (songDetail != null) {
            //String songName=songDetail.getDisplay_name();
            int type = info.getIndex();
            Log.e("video2", "bb" + type);
            LastSongInfo songInfo = new LastSongInfo(context);
            if (type == 0) {
                Log.e("video", "bb" + type);
                songInfo.saveLastSong(getPlayingSongDetail());
                songInfo.saveLastSongListType(type);
                songInfo.saveLastAlbID(id);
                songInfo.saveLastPosition(currentPlaylistNum);
                songInfo.saveLastPath(path);
            }
        }
