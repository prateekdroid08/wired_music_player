 ...
 */
package com.avfplayer.phonemidea;
...
//                sortOrder = MediaStore.Audio.Media.DATE_ADDED + " desc";
                ...
                songsList = getSongsFromCursor(cursor, index);
                break;
            ...
            case Gener:
                ...
                songsList = getSongsFromCursor(cursor, 0);
                break;
            ...
            case Playlist:
                cursor = PlaylistPlayTableHelper.getInstance(context).getFavoriteSongList(""+id);
                songsList = getSongsFromSQLDBCursor(cursor);
                break;
    ...
    private ArrayList<SongDetail> getSongsFromCursor(Cursor cursor, int index) {
        ...
        try {
            if (cursor != null && cursor.getCount() >= 1) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    SongDetail mSongDetail = new SongDetail(ID, album_id, ARTIST, TITLE, Path, DISPLAY_NAME, DURATION, 0, index);
        ...
        try {
            if (cursor != null && cursor.getCount() >= 1) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    SongDetail mSongDetail = new SongDetail((int) ID, (int) album_id, ARTIST, TITLE, Path, DISPLAY_NAME, "" + (Long.parseLong(DURATION) * 1000), 0, 0);
                    generassongsList.add(mSongDetail);
                }
            }
            closeCrs();
    ...
    public static void runOnUIThread(Runnable runnable, long delay) {
        if (delay == 0) {
            ApplicationAVFPlayer.applicationHandler.post(runnable);
        } else {
            ApplicationAVFPlayer.applicationHandler.postDelayed(runnable, delay);
        }
    }
