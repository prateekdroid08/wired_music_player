...
public class ObservableScrollView extends ScrollView implements Scrollable {
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		if (mPrevScrollY < t) {
			...
			// Before Android 4.4, onTouchEvent calls onScrollChanged directly
			// for ACTION_MOVE,
			// which makes mScrollState always STOP when onUpOrCancelMotionEvent
			// is called.
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			// flag initializations which are related to ACTION_DOWN events
			// should be executed.
			// Because if the ACTION_DOWN is consumed by children and only
			// ACTION_MOVEs are
			// passed to parent (this view), the flags will be invalid.
			// Also, applications might implement initialization codes to
			// onDownMotionEvent,
			// so call it here.
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getCurrentScrollY() - diffY <= 0) {
				...
				if (mIntercepted) {
					// Already dispatched ACTION_DOWN event to parents, so stop
					// here.
					return false;
				}
