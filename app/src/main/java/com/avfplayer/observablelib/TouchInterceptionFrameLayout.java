 ...
 */
package com.avfplayer.observablelib;
...
/**
 * A layout that delegates interception of touch motion events. This layout is
 * provided to move the container of Scrollable views using scroll position.
 * Please note that this class overrides or uses touch events API such as
 * onTouchEvent, onInterceptTouchEvent and dispatchTouchEvent, so be careful
 * when you handle touches with this layout.
 */
public class TouchInterceptionFrameLayout extends FrameLayout {
	...
	public interface TouchInterceptionListener {
		 ...
		 *            Motion event.
		 * @param moving
		 *            True if this event is ACTION_MOVE type.
		 * @param diffX
		 *            Difference between previous X and current X, if moving is
		 *            true.
		 * @param diffY
		 *            Difference between previous Y and current Y, if moving is
		 *            true.
		 ...
		 *            Motion event.
		 * @param diffX
		 *            Difference between previous X and current X.
		 * @param diffY
		 *            Difference between previous Y and current Y.
		 */
		void onMoveMotionEvent(MotionEvent ev, float diffX, float diffY);

		/**
		 * Called if the up (or cancel) motion event is intercepted by this
		 * layout.
		 *
		 * @param ev
		 *            Motion event.
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		// Whether we should intercept or not is kept for the later event
		// handling.
		...
		case MotionEvent.ACTION_MOVE:
			// ACTION_MOVE will be passed suddenly, so initialize to avoid
			// exception.
			if (mInitialPoint == null) {
				mInitialPoint = new PointF(ev.getX(), ev.getY());
			}

			// diffX and diffY are the origin of the motion, and should be
			// difference
		...
		if (mTouchInterceptionListener != null) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mIntercepting) {
					...
					// we should generate ACTION_DOWN event with current
					// position.
					...
					// Therefore we set pending flag to true as if this is a
					// down motion event.
					mDownMotionEventPended = true;

					// Whether or not this event is consumed by the listener,
					// assume it consumed because we declared to intercept the
					// event.
					return true;
				} else {
					...
					// Reserve children's click cancellation here if they've
					// already canceled
					mChildrenEventsCanceled = false;
				}
				break;
			...
			case MotionEvent.ACTION_CANCEL:
				...
				// whether or not this layout intercepted the consecutive motion
				// events.
				if (!mChildrenEventsCanceled) {
					mChildrenEventsCanceled = true;
					if (mDownMotionEventPended) {
	...
	/**
	 * Duplicate touch events to child views. We want to dispatch a down motion
	 * event and the move events to child views, but calling
	 * dispatchTouchEvent() causes StackOverflowError. Therefore we do it
	 * manually.
	 *
	 * @param ev
	 *            Motion event to be passed to children.
	 * @param pendingEvents
	 *            Pending events like ACTION_DOWN. This will be passed to the
	 *            children before ev.
	 */
